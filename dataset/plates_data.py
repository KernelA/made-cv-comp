...
class DetectionDataset(data.Dataset):
    ...
    def _prepare_data(self):
        for plate_info in self.marks:
            img_path = os.path.join(self.img_folder, plate_info["file"])

            with Image.open(img_path) as image:
                height, width = image.height, image.width

            box_coords = plate_info['nums']
            boxes = []
            ...
            self._plates_info.append(target)
    ...
    def __getitem__(self, index):
        ...
        return img, self._plates_info[index]
